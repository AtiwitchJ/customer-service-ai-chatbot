...
`latta-csbot-admin` is the administrative interface and document processing system for the CP ALL Customer Service AI Chatbot. It provides:
...
2. **latta-csbot-database** running (Supabase, Redis)
3. **Ollama** server (external or from latta-csbot-user)
...
2. **latta-csbot-llm** (or external Ollama)
   - LLM inference
   - Embedding generation
   - Vision models
